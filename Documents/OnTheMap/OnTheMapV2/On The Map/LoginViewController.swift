...
class LoginViewController: UIViewController,FBSDKLoginButtonDelegate,UINavigationControllerDelegate {
    ...
    @IBAction func basicLoginButtonTouch(sender: AnyObject) {
        self.view.endEditing(true)
        UserUdacity.sharedInstance().authenticateBasicLoginWithViewController(self) { (success, errorString) in
            if success {
                self.completeLogin()
            } else {
    ...
    func loginButton(loginButton: FBSDKLoginButton!, didCompleteWithResult result: FBSDKLoginManagerLoginResult!, error: NSError!) {
        indicator(true)
        UserUdacity.sharedInstance().authenticateFacebookLoginWithViewController(self) { (success, errorString) in
            if success {
                self.completeLogin()
            } else {
                self.displayMessage(errorString!)
