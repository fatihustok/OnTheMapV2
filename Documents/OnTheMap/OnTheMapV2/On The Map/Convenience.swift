...
extension UserUdacity {
        ...
        if FBSDKAccessToken.currentAccessToken() != nil {
            self.getSessionFacebookID(FBSDKAccessToken.currentAccessToken().tokenString){ result, errorString in
                if let _ = errorString{
                    ...
                    if (result != nil) {
                        self.getPublicUserData(UserUdacity.sharedInstance().uniqueKey!)  { account,errorString in
    ...
    func getSessionID(username: String,password: String, completionHandler: (result: String?, error: NSError?) -> Void) {
        ...
        let udacityBody: [String:AnyObject] = [UserUdacity.JSONBody.Username: username, UserUdacity.JSONBody.Password : password ]
        let jsonBody : [String:AnyObject] = [ UserUdacity.JSONBody.Udacity: udacityBody ]
        ...
        _ = taskForPOSTMethod(mutableMethod,parse:false, parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let results = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.Session)?.valueForKey(UserUdacity.JSONResponseKeys.Id) as? String {
                    self.sessionID = results // Setting the session ID
                    if let key = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.Account)?.valueForKey(UserUdacity.JSONResponseKeys.Key) as? String{
                        self.uniqueKey = key
                    }
    ...
    func getSessionFacebookID(access_token: String, completionHandler: (result: String?, error: NSError?) -> Void) {
        ...
        let mutableMethod : String = Methods.AuthenticationSessionNew
        let udacityBody: [String:AnyObject] = [ UserUdacity.JSONResponseKeys.AccessToken: access_token ]
        let jsonBody : [String:AnyObject] = [ UserUdacity.JSONResponseKeys.FacebookMobile: udacityBody ]
    ...
    func getPublicUserData(uniqueKey: String,completionHandler: (result: Student?, error: String?) -> Void) {
        ...
        taskForGETMethod(method,parse: false, parameters: parameters) { JSONResult, error in
            ...
            if let _ = error{
                ...
                if let lastname = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.User)?.valueForKey(UserUdacity.JSONResponseKeys.Last_Name) as? String {
                    if let firstname = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.User)?.valueForKey(UserUdacity.JSONResponseKeys.First_Name) as? String {
                        if let uniqueKey = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.User)?.valueForKey(UserUdacity.JSONResponseKeys.Key) as? String {
    ...
    func getStudentLocations(let limit limit:Int, let skip:Int,completionHandler: (result: [Student]?, error: String?) -> Void){
        ...
        let method = Methods.StudentLocations + UserUdacity.escapedParameters(parameters)
        /* 2. Make the request */
        taskForGETMethod(method,parse: true, parameters: nil) { JSONResult, error in
            ...
            if let _ = error{
                ...
                if let results = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.Results) as? [[String : AnyObject]] {
            ...
            if let _ = error{
                ...
                if let results = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.Results) as? [[String : AnyObject]] {
                    ...
                    if(results.count > 0){
                        ...
                        UserUdacity.sharedInstance().account = student
                    }else{
                        student = nil
                    }
    ...
    func saveAccountLocation(account:Student,completionHandler: (result: Bool?, error: NSError?) -> Void) {
        ...
        let jsonBody: [String:AnyObject] = [ UserUdacity.JSONBody.uniqueKey: account.uniqueKey,UserUdacity.JSONBody.firstName:account.firstName,UserUdacity.JSONBody.lastName:account.lastName,UserUdacity.JSONBody.mapString:account.mapString!,UserUdacity.JSONBody.mediaURL:account.mediaURL!,UserUdacity.JSONBody.latitude:account.latitude!,UserUdacity.JSONBody.longitude:account.longtitude! ]
        ...
        _ = taskForPOSTMethod(method, parse: true,parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let _ = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.ObjectID) as? String {
    ...
    func updateAccountLocation(account:Student,completionHandler: (result: Bool?, error: NSError?) -> Void) {
        ...
        _ = taskForPUTMethod(method,parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let _ = JSONResult.valueForKey(UserUdacity.JSONResponseKeys.UpdatedAt) as? String {
                    completionHandler(result: true, error: nil)
                } else {
